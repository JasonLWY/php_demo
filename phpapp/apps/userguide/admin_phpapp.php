...
class UserGuideManageControls extends PHPAPP{
	...
	function ListAction(){
		  ...
		  if(!empty($this->POST['Submit'])){
			   ...
			   if($this->Delete('userguide'," WHERE sid IN($ids)")){
			   ...
			   echo $this->Refresh($refresh,$this->MakeGetParameterURL());
		         ...
		         $orderarr=array(
						  ...
						  array('order'=>'a.subject','name'=>'引导名称'),
		         ...
		         $ajaxpage=new Pages(PHPAPP::$config['sys_operation_page'],$this->GET['page'],$this->MakeGetParameterURL(),"SELECT a.*,b.name_phpapp AS appname FROM  ".$this->GetTable('userguide')." AS a JOIN ".$this->GetTable('apps')." AS b ON a.appid=b.id_phpapp $order");
	...
	function AddAction(){
		 ...
		 if(!empty($this->POST['Submit'])) {
					  ...
					  $newid=$this->Insert('userguide',$this->POST,array());
	...
	function EditAction(){
		 ...
		 if(!empty($this->POST['Submit'])) {
				 ...
				 $yes=$this->Update('userguide',$this->POST,array(),"WHERE sid='$id'");
				 ...
				 $manage=$this->GetMysqlOne('*'," ".$this->GetTable('userguide')." WHERE sid='$id'");
