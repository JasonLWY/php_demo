...
class RealNameCompanyManageControls extends PHPAPP{
	...
	function ListAction(){
		    ...
		    if(!empty($this->POST['Failure'])){
			     ...
			     $yes=$this->Update('member_company_certificate',array('status'=>6,'errorinfo'=>$errorinfo),array(),"WHERE uid IN($ids)");
			     ...
			     if($yes){
					  ...
					  if($myinfoarr){
							  ...
							  foreach($myinfoarr as $myinfo){
							         ...
							         $certificates=$this->SetCertificateIcon($myinfo['certificate'],'Company',1);
		        ...
		        $this->Update('member_company_certificate',array('pay'=>1),array(),"WHERE uid IN($ids)");
		        ...
		        $this->Update('member_company_certificate',array('pay'=>0),array(),"WHERE uid IN($ids)");
				...
				echo $this->Refresh($refresh,$this->MakeGetParameterURL());		  
			     ...
			     $yes=$this->Update('member_company_certificate',array('status'=>5),array(),"WHERE uid IN($ids)");
				 ...
				 if($this->Delete('member_company_certificate'," WHERE uid IN($ids)")){
		         ...
		         $ajaxpage=new Pages(PHPAPP::$config['sys_operation_page'],$this->GET['page'],$this->MakeGetParameterURL(),"SELECT a.*,b.username,c.mobile FROM  (".$this->GetTable('member_company_certificate')." AS a JOIN ".$this->GetTable('member')." AS b ON a.uid=b.uid) JOIN ".$this->GetTable('member_info')." AS c ON a.uid=c.uid $wheresql $order");
	...
	function ShowAction(){
		    ...
		    $certificate=$this->GetMysqlOne('*'," ".$this->GetTable('member_company_certificate')." WHERE uid='$id'");
	...
	function EditAction(){
		  ...
		  if($this->POST['Submit']){
				...
				if($id>0){
					  ...
					  $this->Update('member_company_certificate',$this->POST,array()," WHERE uid='$id'");
