...
class RealNameCompanyMemberControls extends PHPAPP{
	...
	function __construct(){	 
		 ...
		 if(!PHPAPP::$config['realcompanyisopenmobile']){
			   //mobile
	...
	public function RealNameVerifyAction(){
		   ...
		   if($this->IsSQL('member',"WHERE uid='$this->uid' AND usertype=2")){
		
		
				  include_once(Core.'/class/filter_class_phpapp.php');
				  
				  $certificate=$this->GetMysqlOne('*'," ".$this->GetTable('member_company_certificate')." WHERE uid='$this->uid'");
				  ...
				  if($this->GET['op']==1){
						...
						$this->Update('member_company_certificate',array('status'=>1),array(),"WHERE uid='$this->uid' ");
						...
						if($this->POST['type']==1){
							  ...
							  $string=new CharFilter($bankcard1);
							   ...
							   if($checknum==3){
									  ...
									  if(!$this->IsSQL('member_company_certificate',"WHERE uid='$this->uid'")){
										  
											 $this->Insert('member_company_certificate',array('bankid'=>$this->POST['bankid1'],'bankcard'=>$bankcard1,'bankaddress'=>$bankaddress,'type'=>1,'bankname'=>$bankname,'dateline'=>$this->NowTime(),'uid'=>$this->uid,'status'=>2),array());
									  }else{
										  
											$this->Update('member_company_certificate',array('bankid'=>$this->POST['bankid1'],'bankcard'=>$bankcard1,'bankaddress'=>$bankaddress,'bankname'=>$bankname,'type'=>1,'status'=>2),array(),"WHERE uid='$this->uid' ");
									  }
								   
							   }else{
									echo $this->Refresh($Error,'member.php?app=16&action=1&op=1');
									exit();
							   }
							  ...
							  $string=new CharFilter($bankcard2);
							   ...
							   if($checknum==1){
									  ...
									  if(!$this->IsSQL('member_company_certificate',"WHERE uid='$this->uid'")){
											 ...
											 $this->Insert('member_company_certificate',array('bankid'=>$this->POST['bankid2'],'bankcard'=>$bankcard2,'type'=>2,'dateline'=>$this->NowTime(),'uid'=>$this->uid,'status'=>2),array('Submit'),array());
									  }else{
										  
											$this->Update('member_company_certificate',array('bankid'=>$this->POST['bankid2'],'bankcard'=>$bankcard2,'type'=>2,'status'=>2),array(),array(),"WHERE uid='$this->uid' ");
									  }
						...
						$this->Update('member_company_certificate',array('status'=>2),array(),"WHERE uid='$this->uid' ");
		
						
				  }elseif($this->GET['op']==3){
							
								$this->Update('member_company_certificate',array('status'=>3),array(),"WHERE uid='$this->uid' ");
							  
								$checknum=0;
								
								
								$company=$this->str($this->POST['company'],50,1,0,1,0,1);
							  
							   $string=new CharFilter($company);
							  
							   if(empty($company)){
									 $Error='请输入公司名称!</br>'; 
							   }elseif($string->CheckLength(2)){ 
									 $Error='对不起！,公司名称不能少2个字!</br>';
							   }else{
									 $checknum+=1;
							   }
							   
							   
							   $businessnumber=$this->str($this->POST['businessnumber'],50,1,0,1,0,1);
							  
							   $string=new CharFilter($businessnumber);
							  
							   if(empty($businessnumber)){
									 $Error='请输入工商注册号!</br>'; 
							   }elseif($string->CheckLength(2)){ 
									 $Error='对不起！,工商注册号不能少2个字!</br>';
							   }else{
									 $checknum+=1;
							   }
							   ...
							   if($_FILES['businesslicense']['size']>0){
								     if($_FILES['businesslicense']['size']> intval(PHPAPP::$config['oneimageuploadsize'])){
											$errors='对不起!您上传的营业执照不能超过 '.(PHPAPP::$config['oneimageuploadsize']/1024).'KB,请重新上传！'; 										   
											$this->Refresh($errors,'member.php?app=16&action=1&op=2');							   
									 }
									 $checknum+=1;
									 $upload=new UploadPhoto($_FILES['businesslicense'],$certificate['businesslicense']);
									 $businesslicense=$upload->CheckUpload();
									 
							   }else{
									if($certificate['businesslicense']>0){
										 $businesslicense=$certificate['businesslicense'];
										 $checknum+=1;
									}else{
										 $Error= '对不起！,请上传营业执照(副本)!</br>';
									}
							  ...
							  if($checknum==7){
									  ...
									  $this->Update('member_company_certificate',array('company'=>$company,'businessnumber'=>$businessnumber,'realname'=>$realname,'idnumber'=>$idnumber,'businesslicense'=>$businesslicense,'frontphoto'=>$frontphoto,'rearphoto'=>$rearphoto),array(),"WHERE uid='$this->uid' ");
									 
									 
									 
							  }else{
									echo $this->Refresh($Error,'member.php?app=16&action=1&op=2');
									exit();
							  }
							...
							if($this->IsSQL('member_company_certificate',"WHERE uid='$this->uid' AND frontphoto>0 AND rearphoto>0  ")){
								  
								  
								  
									if(!$this->IsSQL('member_company_certificate',"WHERE uid='$this->uid' AND status=4")){
										   ...
										   $this->Update('member_company_certificate',array('status'=>4,'verifymoney'=>floatval($verifymoney)),array(),"WHERE uid='$this->uid' ");
							...
							if($this->IsSQL('member_company_certificate',"WHERE uid='$this->uid' AND status=4 AND frontphoto>0 AND rearphoto>0")){
								
		
								  if(!empty($this->POST['verifymoney'])){
										
										$verify=$this->GetMysqlOne('verifymoney,errornum'," ".$this->GetTable('member_company_certificate')." WHERE uid='$this->uid'");
										...
										if($verify['verifymoney']==$this->POST['verifymoney']){
											   ...
											   if($certificate['price_phpapp']>0){
												...
												$this->Update('member_company_certificate',array('status'=>5),array(),"WHERE uid='$this->uid' ");
												
												
												$myinfo=$this->GetMysqlOne('certificate',"  ".$this->GetTable('member_info')." WHERE uid='$this->uid' ");
													
		
												$certificates=$this->SetCertificateIcon($myinfo['certificate'],'Company');
												...
												new UnionResult(array('uid'=>$this->uid,'app'=>$this->app,'appid'=>$this->app,'rebate'=>floatval(PHPAPP::$config['companycertificatemoney']),'subject'=>'企业实名认证'));
											  ...
											  if($verify['errornum']>=3){
													...
													$this->Update('member_company_certificate',array('status'=>6),array(),"WHERE uid='$this->uid' ");
													...
													$this->Update('member_company_certificate',array('errornum'=>$verify['errornum']),array(),"WHERE uid='$this->uid' ");
													echo '验证金额有误!';
								  ...
								  exit();
									...
									echo $this->Refresh($Error,'member.php?app=16&action=1');
									exit();
							}	
				  ...
				  include $this->Template('realcompany');
				  
		   }else{
			   
			     echo $this->Refresh('对不起！个人用户不能进行企业实名认证！','member.php?app=12');
		   }
