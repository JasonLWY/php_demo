...
//Manage
class TaskGrabManageControls extends MyTaskGrab{
	...
	public function TaskAdminAction(){
			  ...
			  if($wheresql){
				    $wheresql.=' AND a.appid=83 ';
			  }else{
				    $wheresql=' WHERE a.appid=83 ';
			  }
			 ...
			 if(!empty($this->POST['Delay'])){   //延期
	               ...
	               if($this->uid==1){
					 ...
					 $orderarr=array(
							  ...
							  array('order'=>'a.money','name'=>'预付金'),
							  array('order'=>'a.price2','name'=>'赏金范围'),
							  array('order'=>'a.subject','name'=>'标题'),
							  array('order'=>'a.draft_success','name'=>'投标/中标'),
					 ...
					 $ajaxpage=new Pages(PHPAPP::$config['sys_operation_page'],$this->GET['page'],$this->MakeGetParameterURL(),"SELECT a.*,b.username,c.cid FROM  (".$this->GetTable('task')." AS a LEFT JOIN  ".$this->GetTable('member')." AS b ON a.uid=b.uid) LEFT JOIN ".$this->GetTable('task_order')." AS c ON a.tid=c.tid $wheresql GROUP BY a.tid $order");
	...
	public function DraftAdminAction(){
		      ...
		      if(!empty($this->POST['Failure'])){
					...
					$this->Update('task_draft',array('process'=>2),array(),"WHERE did IN($ids)");
					
					$weiboorderarray=$this->GetMysqlArray('*'," ".$this->GetTable('task_order')." WHERE did IN($ids) ");    
					
					if($weiboorderarray){
						
						  foreach($weiboorderarray as $value){
							  
							      @$this->Update('consume',array('process'=>6),array(),"WHERE serial='$value[serial]'");
						  }
					...
					$this->Update('task_draft',array('process'=>0),array(),"WHERE did IN($ids)");
					...
					$this->Update('task_draft',array('process'=>1),array(),"WHERE did IN($ids)");
					...
					if($weiboorderarray){
						  ...
						  foreach($weiboorderarray as $value){
							      ...
							      @$this->Update('consume',array('process'=>5),array(),"WHERE serial='$value[serial]'");
						  }
	...
	public function AddAllowAction(){
			     ...
			     if(!empty($this->POST['Submit'])) {
						...
						if($this->IsSQL('task_grab_usergroup',"WHERE gid='$gid'")){
							
	
							   $yes=$this->Update('task_grab_usergroup',$this->POST,array(),"WHERE gid='$gid'");
							   ...
							   exit();
								...
								$gid=$this->Insert('task_grab_usergroup',$this->POST);
	...
	public function EditAllowAction(){
			     ...
			     if(!empty($this->POST['Submit'])) {
					    ...
					    $gid=$this->GET['id'];
					    $nowvalue=$this->GetMysqlOne('*'," ".$this->GetTable('task_grab_usergroup')." WHERE gid='$gid'");
	...
	public function AllowAdminAction(){
				 ...
				 if(!empty($this->POST['Submit'])) {
					     ...
					     if($this->Delete('task_grab_usergroup'," WHERE gid IN($ids)")){
						 ...
						 $ajaxpage=new Pages(PHPAPP::$config['sys_operation_page'],$this->GET['page'],$this->MakeGetParameterURL(),"SELECT a.*,b.gid AS groupid FROM  ".$this->GetTable('usergroup')." AS a  LEFT JOIN ".$this->GetTable('task_grab_usergroup')." AS b ON a.gid=b.gid $order");
	...
	public function CopyRightAction(){
		    ...
		    include_once(APPS.'/taskgrab/version_phpapp.php');
